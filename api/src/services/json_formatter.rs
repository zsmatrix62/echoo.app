...
impl JsonFormatterService for JsonFormatterServiceImpl {
    async fn get_random_json(&self, _: Request<()>) -> Result<Response<OutGetRandomJson>, Status> {
        ...
        serde_json::to_string(&fake_json_data)
            ...
            .map(|data| Response::new(
                OutGetRandomJson { input_sample: data }))
    }

    async fn validate_json(
        &self,
        request: Request<InValidateJson>,
    ) -> Result<Response<OutValidateJson>, Status> {
        ...
        Ok(Response::new(OutValidateJson {
            errors: validation_errors.into_iter().map(|v_err| {
                ValidationError {
