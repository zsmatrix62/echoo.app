...
import type { ToolSettingItem, ToolSettings } from '../../data/types/tool-config';
...
export class ToolSettingsService<K extends string, T extends ToolSettings<K> = ToolSettings<K>> {
	...
	static fromDefaultSettings<K extends string>(settingValue: ToolSettings<K>): ToolSettingsService<K> {
	...
	private listenToQueryParams() {
		...
		this.art.queryParams.subscribe((params) => {
			Object.keys(params).forEach((queryKey) => {
				...
				// @ts-ignore
				const settingItem = <ToolSettingItem<unknown>>this.propertToolSettings[queryKey];
	...
	private resetDefault() {
		...
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		const defualtAppConfig: { [key: string]: any } = this.storeService.get(this.appConfigKey) ?? {};
	...
	getAll(): object {
		...
		Object.keys(this.propertToolSettings).forEach((key) => {
			// @ts-ignore
			settings[key] = this.propertToolSettings[key].value;
		});
		return settings;
	}
}
