...
const VerCopyItem = (props: { title: string; content: string }) => {
  return (
    <Space vertical style={{ marginTop: "10px", width: "100%" }} align="start">
...
const LeftBlock = () => {
  ...
  const [decodeUuidRes, setDecodeUuidRes] = useObservableState<
    UUIDRes | undefined
  >((obs) => obs, undefined);

  const [decodeUuidResV1Tm, setDecodeUuidResV1Tm] = useObservableState<
    [string, string]
  >((obs) => obs, ["", ""]);

  const [decodeUuidUpperCased, setDecodeUuidUpperCased] = useObservableState(
    (obs) => {
      obs.subscribe((checked) => {
        decodeUuidUpperCased$.next(checked);
      });
      return obs;
    },
    false
  ...
  const [decodeInputValue, setDecodeInputValue] = useObservableState<
    string | undefined
  >((obs) => obs, undefined);
  ...
  const sampleUUID = () => {
    import("wasm-api").then((wasm) => {
      try {
        const uuid: string = [
          wasm.gen_uuid_v1().std_string,
          wasm.gen_uuid_v4().std_string,
      ...
      >
        <div>
          ...
          <Row>
            {" "}
            <VerCopyItem
              title="Standard String Format"
              content={decodeUuidRes?.std_string ?? ""}
            ></VerCopyItem>{" "}
          </Row>
          <Row>
            {" "}
            <VerCopyItem
              title="URN Format"
              content={decodeUuidRes?.urn_string ?? ""}
            ></VerCopyItem>{" "}
          </Row>
          <Row>
            {" "}
            <VerCopyItem
              title="Simple Format"
              content={decodeUuidRes?.simple_string ?? ""}
            ></VerCopyItem>{" "}
          </Row>
          <Row>
            {" "}
            <VerCopyItem
              ...
              content={decodeUuidRes?.integer_value ?? ""}
            ></VerCopyItem>{" "}
          </Row>
          <Row>
            {" "}
            <VerCopyItem
              ...
              content={
                decodeUuidRes
                  ? `v${decodeUuidRes?.version_num ?? ""} (${
                      decodeUuidRes?.version ?? ""
                    })`
                  : ""
              }
            ></VerCopyItem>{" "}
          ...
          <Row>
            ...
            <VerCopyItem
              title="Raw Contents"
              content={decodeUuidRes?.raw_content ?? ""}
            ></VerCopyItem>{" "}
          </Row>
          ...
          {decodeUuidRes?.version_num == 1 ? (
            <>
              <Row>
                ...
                <VerCopyItem
                  title="Content - Time"
                  content={decodeUuidResV1Tm[0]}
                ></VerCopyItem>{" "}
              </Row>
              <Row>
                {" "}
                <VerCopyItem
                  title={"Content - Clock ID"}
                  content={decodeUuidResV1Tm[1]}
                ></VerCopyItem>{" "}
              </Row>
              <Row>
                {" "}
...
const Uuid35Input = (props: { cb: (uuid: string, name: string) => void }) => {
  ...
  const [_, setNs] = useObservableState(
    (obs) => {
      obs.subscribe((ns) => {
        props.cb(ns[0], ns[1]);
      });
...
const RightBlock = () => {
  ...
  const IDsTextArea = (props: { rows: number | string; content: string }) => {
    return (
      <TextArea
        ...
        style={{ height: inputHeight + "px", overflow: "auto" }}
  ...
  const [visibleTabs, setVisibleTabs] = useObservableState<string>(
    (obs) => obs,
    "123"
  );
  ...
  const [setUuids, uuids$] = useObservableCallback<UUIDRes[], UUIDRes[]>(
    (e$) => e$
  );
  ...
  const [uuidString, setUuidString] = useObservableState<string>(
    (obs) => obs,
    ""
  );
  const [enableUuidCount, setEnableUuidCount] = useObservableState<boolean>(
    (obs) => obs,
    true
  );
  ...
  const [genVersion, setGenVersion] = useObservableState(
    (obs) => obs,
    "uuidv1"
  );

  const [uppercase, setUppserCase] = useObservableState((obs) => obs, false);
  const [onUppercaseChanged, uppercase$] = useObservableCallback<
    boolean,
    CheckboxEvent
  >((e) => {
    return e.pipe(map((ie) => !!ie.target.checked));
  });
  useSubscription(uppercase$, (checked) => {
    setUppserCase(checked);
    setUuidString(
      checked ? uuidString.toUpperCase() : uuidString.toLowerCase()
    );
  });

  const [setActiveIndex, activeIndexChanged$] = useObservableCallback<
    string,
    string
  >(($e) => $e);
  ...
  const fillTextArea = (uuids: UUIDRes[], activeKey?: string) => {
    ...
    switch (activeKey || key) {
      case "1":
        setUuidStrings(uuids.map((u) => u.std_string));
        break;
      case "2":
        setUuidStrings(uuids.map((u) => u.urn_string));
        break;
      case "3":
        setUuidStrings(uuids.map((u) => u.simple_string));
        break;
    }
  };
  ...
  return (
    <Row style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      <Row type="flex" style={{ justifyContent: "space-between" }}>
        <Col>
          {" "}
          <Typography.Text>Generate new IDs</Typography.Text>{" "}
        </Col>
        <Col>
          <Space>
            <Select
      ...
      </Row>
      {
        <Row
          style={
            !["uuidv3", "uuidv5"].includes(genVersion)
              ? { display: "none" }
              : {}
          }
        >
      ...
      <Row
        style={{ paddingTop: "10px", flex: 1, boxSizing: "border-box" }}
        className="input-tab"
      >
        <Tabs
          tabPosition="left"
          ...
          {Object.keys(tabKeyMap).map((k) => {
            return (
              <TabPane
                ...
                itemKey={k}
                key={k}
                disabled={!visibleTabs.includes(k)}
                style={{ height: "100%" }}
              >
                <IDsTextArea
                  rows={uuidCount}
    ...
    <Row style={{ height: "100%" }} className="uuid-generator-container">
      <Col span={8} style={{ padding: "16px 10px 10px 10px", height: "100%" }}>
        <Card
          ...
          bodyStyle={{ flex: 1, boxSizing: "border-box", overflow: "hidden" }}
        >
          <LeftBlock />
        </Card>
      </Col>
